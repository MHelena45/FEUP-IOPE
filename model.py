...
M = 10; # Value greater than the number of needed vehicles for sure
...
# for vessel in range(1, M):
    # for tripType in S1:
        # tA[vessel,tripType,1]=model.addVar(vtype="I", name="a(%s,%s,%s)" % (vessel,tripType,1))
    # for tripType in S2:
        # tA[vessel,tripType,2]=model.addVar(vtype="I", name="a(%s,%s,%s)" % (vessel,tripType,2))
...
'''
model.addConstr(dFull1 == quicksum(a[vessel,1,1,importType, export] * d["Doce","Sky"] * 2 for vessel in range(1, M) for importType in PT for export in PT) +    # trip: Doce – Sky – Doce
    quicksum(a[vessel,2,1,importType, export] * d["Doce","Mars"] * 2  for vessel in range(1, M) for importType in PT for export in PT) +     # trip: Doce – Mars – Doce
    quicksum(a[vessel,3,1,importType, export] * (d["Doce","Sky"] + d["Mars","Doce"])  +       # trip: Doce – Sky – Mars - Doce
...
model.addConstr(dFull2 == quicksum(a[vessel,1,2,importType, export] * d["Doce","Sky"] * 2 for vessel in range(1, M) for importType in PT for export in PT) +           # trip: Doce – Sky – Doce
    quicksum(a[vessel,2,2,importType, export] * d["Doce","Mars"] * 2 for vessel in range(1, M) for importType in PT for export in PT) +                   # trip: Doce – Mars – Doce
    quicksum(a[vessel,3,2,importType, export] * (d["Doce","Sky"] + d["Mars","Doce"])  +         # trip: Doce – Sky – Mars - Doce
...
# Bom – Mars – Bom [6,1] and [6,2]; Bom – Mars - Moon - Bom [18, 1]; Bom – Mars - Sky - Bom [8, 1] and [8, 2]
    model.addConstr(quicksum(a[vessel,6,1,importType,"Wheat"] + a[vessel,18,1,importType,"Wheat"] + a[vessel,8,1,importType,"Wheat"] for importType in PT)* 35000 + 
        quicksum(a[vessel,6,2,importType,"Wheat"] + a[vessel,8,2,importType,"Wheat"] for importType in PT) * 70000 >= 20000, "c7")

# 30 000 go to Sky 
# Bom – Sky – Bom [5, 1] and [5, 2]; Bom – Sky - Moon – Bom [15, 1]; Bom – Sky - Mars – Bom [7, 1] and [7, 2]
    model.addConstr(quicksum(a[vessel,5,1,importType,"Wheat"] + a[vessel,15,1,importType,"Wheat"] + a[vessel,7,1,importType,"Wheat"] for importType in PT)* 35000 
        + quicksum(a[vessel,5,2,importType,"Wheat"] + a[vessel,7,2,importType,"Wheat"] for importType in PT) * 70000 >= 30000, "c8")


# Bom import of iron: 50 000 
    model.addConstr(quicksum(a[vessel,5,1,"Iron",export] + a[vessel,17,1,"Iron",export] + a[vessel,8,1,"Iron",export] + a[vessel,6,1,"Iron",export] + a[vessel,7,1,"Iron",export] + a[vessel,16,1,"Iron",export] for export in PT)* 35000 
        + quicksum(a[vessel,5,2,"Iron",export] + a[vessel,8,2,"Iron",export] + a[vessel,6,2,"Iron",export] + a[vessel,7,2,"Iron",export] for export in PT) * 70000 >= 50000, "c9")
...
# Doce – Mars – Doce [2, 1] and [2, 2]; Doce – Mars – Moon – Doce [13, 1]; Doce – Mars – Sky – Doce [4, 1] and [4, 2]
    model.addConstr(quicksum(a[vessel,2,1,importType,"Corn"] + a[vessel,13,1,importType,"Corn"] + a[vessel,4,1,importType,"Corn"] for importType in PT)* 35000
        + quicksum(a[vessel,2,2,importType,"Corn"] + a[vessel,4,2,importType,"Corn"] for importType in PT) * 70000 >= 10000, "c10")

# 30 000 go to Moon
# Doce – Moon – Doce [9, 1]; Doce – Moon - Mars - Doce [11, 1]; Doce – Moon - Sky - Doce [12, 1]
    model.addConstr(quicksum(a[vessel,9,1,importType,"Corn"] + a[vessel,11,1,importType,"Corn"] + a[vessel,12,1,importType,"Corn"] for importType in PT)* 35000 <= 30000, "c11")
...
# Doce – Sky – Doce [1, 1] and [1, 2]; Doce – Moon - Sky - Doce [12, 1]; Doce – Mars – Sky – Doce [4, 1] and [4, 2]
    model.addConstr(quicksum(a[vessel,1,1,"Copper",export] + a[vessel,12,1,"Copper",export] + a[vessel,4,1,"Copper",export] for export in PT)* 35000 
        + quicksum(a[vessel,1,2,"Copper",export] + a[vessel,4,2,"Copper",export] for export in PT) * 70000 >= 10000, "c12")

# 10 000 comes from Moon
# Doce – Moon – Doce [9, 1]; Doce – Sky - Moon – Doce [10, 1]; Doce – Moon - Mars - Doce [11, 1]
    model.addConstr(quicksum(a[vessel,9,1,"Copper",export] + a[vessel,10,1,"Copper",export] + a[vessel,11,1,"Copper",export] for export in PT)* 35000 >= 10000, "c13")
...
# Doce – Sky – Doce [1, 1] and [1, 2]; Doce – Moon - Sky - Doce [12, 1]; Doce – Mars – Sky – Doce [4, 1] and [4, 2]
    model.addConstr(quicksum(a[vessel,1,1,"Iron",export] + a[vessel,2,1,"Iron",export] + a[vessel,3,1,"Iron",export] + a[vessel,11,1,"Iron",export] + a[vessel,12,1,"Iron",export] + a[vessel,4,1,"Iron",export] for export in PT)* 35000 + 
        quicksum(a[vessel,1,2,"Iron",export] + a[vessel,2,2,"Iron",export] + a[vessel,3,2,"Iron",export] + a[vessel,4,2,"Iron",export] for export in PT) * 70000 >= 20000, "c14")
...
# 10 000 of Corn 
# Doce – Mars – Doce [2, 1] and [2, 2]; Doce – Mars – Moon – Doce [13, 1]; Doce – Mars – Sky – Doce [4, 1] and [4, 2]
    model.addConstr(quicksum(a[vessel,2,1,"Corn",export] + a[vessel,13,1,"Corn",export] + a[vessel,4,1,"Corn",export] for export in PT)* 35000 
        + quicksum(a[vessel,2,2,"Corn",export] + a[vessel,4,2,"Corn",export] for export in PT) * 70000 >= 10000, "c15")

# Sky export Iron: 40000 
# Doce – Sky – Doce [1, 1] and [1, 2]; Doce – Moon - Sky - Doce [12, 1]; Doce – Mars – Sky – Doce  [4, 1] and [4, 2];
# Bom – Sky – Bom [5,1] and [5, 2]; Bom - Moon – Sky - Bom [17, 1]; Bom – Mars - Sky - Bom [8, 1] and [8, 2]
    model.addConstr(quicksum(a[vessel,1,1,importType,"Iron"] + a[vessel,4,1,importType,"Iron"] + a[vessel,5,1,importType,"Iron"] + a[vessel,8,1,importType,"Iron"] + a[vessel,12,1,importType,"Iron"] + a[vessel,17,1,importType,"Iron"]  for importType in PT)* 35000 + 
        quicksum(a[vessel,1,2,importType,"Iron"] + a[vessel,4,2,importType,"Iron"] + a[vessel,5,2,importType,"Iron"] + a[vessel,8,2,importType,"Iron"]  for importType in PT) * 70000 >= 40000, "c16")


# Acabam em Sky – Wheat: 30 000
# Bom – Sky – Bom [5,1] and [5, 2]; Bom – Sky - Moon – Bom [15, 1]; Bom – Sky - Mars – Bom [7,1] and [7,2]
    model.addConstr(quicksum(a[vessel,5,1,"Wheat",export] + a[vessel,15,1,"Wheat",export] + a[vessel,7,1,"Wheat",export] for export in PT)* 35000 + 
        quicksum(a[vessel,5,2,"Wheat",export] + a[vessel,7,2,"Wheat",export] for export in PT) * 70000 >= 30000, "c17")

# Export of Moon -  Copper: 10000
# Doce – Moon – Doce [9, 1]; Doce – Sky - Moon – Doce [10, 1]; Doce – Mars – Moon – Doce [13, 1]
    model.addConstr(quicksum(a[vessel,9,1,"Copper", export] + a[vessel,10,1,"Copper", export] + a[vessel,13,1,"Copper", export] for export in PT)* 35000 >= 10000, "c18")

# Chegam a Moon – Corn: 30 000
# Doce – Moon – Doce [9, 1]; Doce – Moon - Mars - Doce [11, 1]; Doce – Moon - Sky - Doce [12, 1]
    model.addConstr(quicksum(a[vessel,9,1,"Corn", export] + a[vessel,11,1,"Corn", export] + a[vessel,12,1,"Corn", export] for export in PT)* 35000 >= 10000, "c19")
...
for vessel in range(1, M):
    for i in S1:
        model.addConstr(quicksum(a[vessel,i,1,importType, export] for export in PT for importType in PT) == tA[vessel,i,1], "c20")
    for i in S2:
        model.addConstr(quicksum(a[vessel,i,2,importType, export] for export in PT for importType in PT) == tA[vessel,i,1], "c20")
'''
...
model.setObjective(quicksum(t[j,1] * a[i,j,2,importType,export] for i in range(1,M) for j in S2 for importType in PT for export in PT) 
+ quicksum(t[j,2] * a[i,j,2,importType,export] for i in range(1,M) for j in S2 for importType in PT for export in PT), GRB.MINIMIZE)
