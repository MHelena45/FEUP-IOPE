...
# BOM does not export CORN
for vessel in range(1, M):
    for importType in IMP:
        model.addConstr(a[vessel,5,1,importType, "Corn"]  == 0)
        model.addConstr(a[vessel,5,2,importType, "Corn"]  == 0)
        model.addConstr(a[vessel,6,1,importType, "Corn"]  == 0)
        model.addConstr(a[vessel,6,2,importType, "Corn"]  == 0)
        model.addConstr(a[vessel,7,1,importType, "Corn"]  == 0)
        model.addConstr(a[vessel,7,2,importType, "Corn"]  == 0)
        model.addConstr(a[vessel,8,1,importType, "Corn"]  == 0)
        model.addConstr(a[vessel,8,2,importType, "Corn"]  == 0)
        model.addConstr(a[vessel,14,1,importType, "Corn"]  == 0)
        model.addConstr(a[vessel,15,1,importType, "Corn"]  == 0)
        model.addConstr(a[vessel,16,1,importType, "Corn"]  == 0)
        model.addConstr(a[vessel,17,1,importType, "Corn"]  == 0)
...
for vessel in range(1, M) for importType in IMP for export in EXP),"c1")
...
for vessel in range(1, M) for importType in IMP for export in EXP),"c2")
...
model.addConstr(dEmpty1 == quicksum(a[vessel,3,2,importType, export] * d["Sky","Mars"] +        # trip: Doce – Sky – Mars - Doce
    ...
    for vessel in range(1, M) for importType in IMP for export in EXP),"c3")

model.addConstr(dEmpty2 == quicksum(a[vessel,3,2,importType, export] * d["Sky","Mars"] +        # trip: Doce – Sky – Mars - Doce
    a[vessel,4,2,importType, export] * d["Mars","Sky"] +        # trip: Doce – Mars – Sky – Doce
    a[vessel,7,2,importType, export] * d["Sky","Mars"] +        # trip: Bom – Sky - Mars – Bom
    a[vessel,8,2,importType, export] * d["Mars","Sky"]          # trip: Bom – Mars - Sky - Bom
    for vessel in range(1, M) for importType in IMP for export in EXP),"c4")
...
model.addConstr(quicksum(a[vessel,6,1,importType,"Wheat"] + a[vessel,18,1,importType,"Wheat"] + a[vessel,8,1,importType,"Wheat"] 
    for vessel in range(1, M) for importType in IMP)* 35 + 
    quicksum(a[vessel,6,2,importType,"Wheat"] + a[vessel,8,2,importType,"Wheat"] 
    for vessel in range(1, M) for importType in IMP) * 70 >= 20000, "c5")

# 30 000 go to Sky 
# Bom – Sky – Bom [5, 1] and [5, 2]; Bom – Sky - Moon – Bom [15, 1]; Bom – Sky - Mars – Bom [7, 1] and [7, 2]
model.addConstr(quicksum(a[vessel,5,1,importType,"Wheat"] + a[vessel,15,1,importType,"Wheat"] + a[vessel,7,1,importType,"Wheat"] 
    for importType in IMP for vessel in range(1, M))* 35 
    + quicksum(a[vessel,5,2,importType,"Wheat"] + a[vessel,7,2,importType,"Wheat"] 
    for importType in IMP for vessel in range(1, M)) * 70 >= 30000, "c6")
...
model.addConstr(quicksum(a[vessel,5,1,"Iron",export] + a[vessel,17,1,"Iron",export] + a[vessel,8,1,"Iron",export] 
    + a[vessel,6,1,"Iron",export] + a[vessel,7,1,"Iron",export] + a[vessel,16,1,"Iron",export] 
    for vessel in range(1, M) for export in EXP)* 35
    + quicksum(a[vessel,5,2,"Iron",export] + a[vessel,8,2,"Iron",export] + a[vessel,6,2,"Iron",export] + a[vessel,7,2,"Iron",export]
    for vessel in range(1, M) for export in EXP) * 70 >= 50000, "c7")
...
model.addConstr(quicksum(a[vessel,2,1,importType,"Corn"] + a[vessel,13,1,importType,"Corn"] + a[vessel,4,1,importType,"Corn"] 
    for vessel in range(1, M) for importType in IMP)* 35
    + quicksum(a[vessel,2,2,importType,"Corn"] + a[vessel,4,2,importType,"Corn"] 
    for vessel in range(1, M) for importType in IMP) * 70 >= 10000, "c8")

# 30 000 go to Moon
# Doce – Moon – Doce [9, 1]; Doce – Moon - Mars - Doce [11, 1]; Doce – Moon - Sky - Doce [12, 1]
model.addConstr(quicksum(a[vessel,9,1,importType,"Corn"] + a[vessel,11,1,importType,"Corn"] + a[vessel,12,1,importType,"Corn"] 
    for vessel in range(1, M) for importType in IMP)* 35 >= 30000, "c9")
...
model.addConstr(quicksum(a[vessel,1,1,"Copper",export] + a[vessel,12,1,"Copper",export] + a[vessel,4,1,"Copper",export] 
    for vessel in range(1, M) for export in EXP)* 35 
    + quicksum(a[vessel,1,2,"Copper",export] + a[vessel,4,2,"Copper",export] 
    for vessel in range(1, M) for export in EXP) * 70 >= 10000, "c10")

# 10 000 comes from Moon
# Doce – Moon – Doce [9, 1]; Doce – Sky - Moon – Doce [10, 1]; Doce – Moon - Mars - Doce [11, 1]
model.addConstr(quicksum(a[vessel,9,1,"Copper",export] + a[vessel,10,1,"Copper",export] + a[vessel,11,1,"Copper",export] 
    for vessel in range(1, M) for export in EXP)* 35 >= 10000, "c11")
...
# Doce – Sky – Doce [1, 1] and [1, 2]; Doce – Moon - Sky - Doce [12, 1]; Doce – Mars – Sky – Doce [4, 1] and [4, 2]
model.addConstr(quicksum(a[vessel,1,1,"Iron",export] + a[vessel,2,1,"Iron",export] + a[vessel,3,1,"Iron",export] 
    + a[vessel,11,1,"Iron",export] + a[vessel,12,1,"Iron",export] + a[vessel,4,1,"Iron",export] 
    for vessel in range(1, M) for export in EXP)* 35 + 
    quicksum(a[vessel,1,2,"Iron",export] + a[vessel,2,2,"Iron",export] + a[vessel,3,2,"Iron",export] + a[vessel,4,2,"Iron",export] 
    for vessel in range(1, M)  for export in EXP) * 70 >= 20000, "c12")
...
model.addConstr(quicksum(a[vessel,2,1,importType,"Corn"] + a[vessel,13,1,importType,"Corn"] + a[vessel,4,1,importType,"Corn"] 
    for importType in IMP for vessel in range(1, M) )* 35 
    + quicksum(a[vessel,2,2,importType,"Corn"] + a[vessel,4,2,importType,"Corn"] 
    for importType in IMP for vessel in range(1, M) ) * 70 >= 10000, "c13")

# Sky export Iron: 40000 
# Doce – Sky – Doce [1, 1] and [1, 2]; Doce – Moon - Sky - Doce [12, 1]; Doce – Mars – Sky – Doce  [4, 1] and [4, 2];
# Bom – Sky – Bom [5,1] and [5, 2]; Bom - Moon – Sky - Bom [17, 1]; Bom – Mars - Sky - Bom [8, 1] and [8, 2]
model.addConstr(quicksum(a[vessel,1,1,"Iron",export] + a[vessel,4,1,"Iron",export] + a[vessel,5,1,"Iron",export] 
    + a[vessel,8,1,"Iron",export] + a[vessel,12,1,"Iron",export] + a[vessel,17,1,"Iron",export]  
    for vessel in range(1, M) for export in EXP)* 35 + 
    quicksum(a[vessel,1,2,"Iron",export] + a[vessel,4,2,"Iron",export] + a[vessel,5,2,"Iron",export] + a[vessel,8,2,"Iron",export] 
    for vessel in range(1, M)  for export in EXP) * 70 >= 40000, "c14")
...
model.addConstr(quicksum(a[vessel,5,1,importType,"Wheat"] + a[vessel,15,1,importType,"Wheat",] + a[vessel,7,1,importType,"Wheat",] 
    for importType in IMP for vessel in range(1, M) )* 35 + 
    quicksum(a[vessel,5,2,importType,"Wheat",] + a[vessel,7,2,importType,"Wheat",] 
    for importType in IMP for vessel in range(1, M)) * 70 >= 30000, "c15")

# Export of Moon -  Copper: 10000
# Doce – Moon – Doce [9, 1]; Doce – Sky - Moon – Doce [10, 1]; Doce – Mars – Moon – Doce [13, 1]
model.addConstr(quicksum(a[vessel,9,1,"Copper", export] + a[vessel,10,1,"Copper", export] + a[vessel,13,1,"Copper", export] 
    for vessel in range(1, M)  for export in EXP)* 35 >= 10000, "c16")
...
for vessel in range(1, M):
    ...
    model.addConstr(quicksum( t[j,1] * a[vessel,j,1,importType,export] for j in S1 for importType in IMP for export in EXP) <= 345 * 24, "c17(%s,%s)" % (vessel,1))
    #the same for type 2
    model.addConstr(quicksum( t[j,2] * a[vessel,j,2,importType,export] for j in S2 for importType in IMP for export in EXP) <= 345 * 24, "c18(%s,%s)" % (vessel,2))
...
for vessel in range(1, M):
    ...
    model.addConstr(quicksum(a[vessel,j,1,importType,export] for j in S1 for importType in IMP for export in EXP) 
        >= x[vessel,1], "c19(%s,%s)" % (vessel,1)) # if nothing is assigned, the value is 0
    model.addConstr(x[vessel,1] * quicksum(a[vessel,j,1,importType,export] for j in S1 for importType in IMP for export in EXP) 
        >= quicksum(a[vessel,j,1,importType,export] for j in S1 for importType in IMP for export in EXP), "c20(%s,%s)" % (vessel,1)) # if something is assigned, the value is 1
    model.addConstr(quicksum(a[vessel,j,2,importType,export] for j in S2 for importType in IMP for export in EXP) 
        >= x[vessel,2], "c21(%s,%s)" % (vessel,2)) # if nothing is assigned, the value is 0
    model.addConstr(x[vessel,2] * quicksum(a[vessel,j,2,importType,export] for j in S2 for importType in IMP for export in EXP) 
        >= quicksum(a[vessel,j,2,importType,export] for j in S2 for importType in IMP for export in EXP), "c22(%s,%s)" % (vessel,2)) # if something is assigned, the value is 1
    # ensure that a boot x can only be use if the boot x-1 has been used
    if vessel >=2:
        model.addConstr(x[vessel,1] <= x[vessel-1,1], "c23(%s,%s)" % (vessel,1))
        model.addConstr(x[vessel,2] <= x[vessel-1,2], "c24(%s,%s)" % (vessel,2))
...
#model.setObjective(((dFull1/1000) * 50 + (dEmpty1/1000) * 42 + (dFull2/1000) * 40 + (dEmpty2/1000) * 30) * 0.8, GRB.MINIMIZE)
