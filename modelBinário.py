...
x={}
x[1]=model.addVar(vtype="I", name="x(1)")
x[2]=model.addVar(vtype="I", name="x(2)")
...
model.addConstr(quicksum(a[i,6,1,importType,"Wheat"] + a[i,18,1,importType,"Wheat"] + a[i,8,1,importType,"Wheat"] for i in S1 for importType in PT)* 35000 + 
quicksum(a[i,6,2,importType,"Wheat"] + a[i,8,2,importType,"Wheat"] for i in S2 for importType in PT) * 70000 >= 20000, "c7")

# 30 000 go to Sky 
# Bom – Sky – Bom [5, 1] and [5, 2]; Bom – Sky - Moon – Bom [15, 1]; Bom – Sky - Mars – Bom [7, 1] and [7, 2]
model.addConstr(quicksum(a[i,5,1,importType,"Wheat"] + a[i,15,1,importType,"Wheat"] + a[i,7,1,importType,"Wheat"] for i in S1 for importType in PT)* 35000 
+ quicksum(a[i,5,2,importType,"Wheat"] + a[i,7,2,importType,"Wheat"] for i in S2 for importType in PT) * 70000 >= 30000, "c8")


# Bom import of iron: 50 000 
model.addConstr(quicksum(a[i,5,1,"Iron",export] + a[i,17,1,"Iron",export] + a[i,8,1,"Iron",export] + a[i,6,1,"Iron",export] + a[i,7,1,"Iron",export] + a[i,16,1,"Iron",export] for i in S1 for export in PT)* 35000 
+ quicksum(a[i,5,2,"Iron",export] + a[i,8,2,"Iron",export] + a[i,6,2,"Iron",export] + a[i,7,2,"Iron",export] for i in S2 for export in PT) * 70000 >= 50000, "c9")


# 30 000 comes from Mars
# Bom – Mars – Bom [6,1] and [6,2]; Bom – Sky - Mars – Bom [7,1] and [7,2]; Bom - Moon – Mars - Bom [16,1]
#model.addConstr(quicksum(a[i,6,1] + a[i,7,1] + a[i,16,1] for i in S1)* 35000 + quicksum(a[i,6,2] + a[i,7,2] for i in S2) * 70000 >= c[3,1], "c9")
# 40 000 comes from Sky
# Bom – Sky – Bom [5,1] and [5, 2]; Bom - Moon – Sky - Bom [17, 1]; Bom – Mars - Sky - Bom [8, 1] and [8, 2]
#model.addConstr(quicksum(a[i,5,1] + a[i,17,1] + a[i,8,1] for i in S1)* 35000 + quicksum(a[i,5,2] + a[i,8,2] for i in S2) * 70000 >= 40000, "c10")

# Doce export of Corn: 40 000

# 10 000 go to Mars 
# Doce – Mars – Doce [2, 1] and [2, 2]; Doce – Mars – Moon – Doce [13, 1]; Doce – Mars – Sky – Doce [4, 1] and [4, 2]
model.addConstr(quicksum(a[i,2,1,importType,"Corn"] + a[i,13,1,importType,"Corn"] + a[i,4,1,importType,"Corn"] for i in S1 for importType in PT)* 35000
+ quicksum(a[i,2,2,importType,"Corn"] + a[i,4,2,importType,"Corn"] for i in S2 for importType in PT) * 70000 >= 10000, "c10")

# 30 000 go to Moon
# Doce – Moon – Doce [9, 1]; Doce – Moon - Mars - Doce [11, 1]; Doce – Moon - Sky - Doce [12, 1]
model.addConstr(quicksum(a[i,9,1,importType,"Corn"] + a[i,11,1,importType,"Corn"] + a[i,12,1,importType,"Corn"] for i in S1 for importType in PT)* 35000 <= 30000, "c11")
...
model.addConstr(quicksum(a[i,1,1,"Copper",export] + a[i,12,1,"Copper",export] + a[i,4,1,"Copper",export] for i in S1 for export in PT)* 35000 
+ quicksum(a[i,1,2,"Copper",export] + a[i,4,2,"Copper",export] for i in S2 for export in PT) * 70000 >= 10000, "c12")

# 10 000 comes from Moon
# Doce – Moon – Doce [9, 1]; Doce – Sky - Moon – Doce [10, 1]; Doce – Moon - Mars - Doce [11, 1]
model.addConstr(quicksum(a[i,9,1,"Copper",export] + a[i,10,1,"Copper",export] + a[i,11,1,"Copper",export] for i in S1 for export in PT)* 35000 >= 10000, "c13")
...
# Doce – Sky – Doce [1, 1] and [1, 2]; Doce – Moon - Sky - Doce [12, 1]; Doce – Mars – Sky – Doce [4, 1] and [4, 2]
model.addConstr(quicksum(a[i,1,1,"Iron",export] + a[i,2,1,"Iron",export] + a[i,3,1,"Iron",export] + a[i,11,1,"Iron",export] + a[i,12,1,"Iron",export] + a[i,4,1,"Iron",export] for i in S1 for export in PT)* 35000 + 
    quicksum(a[i,1,2,"Iron",export] + a[i,2,2,"Iron",export] + a[i,3,2,"Iron",export] + a[i,4,2,"Iron",export] for i in S2 for export in PT) * 70000 >= 20000, "c14")
...
# 10 000 of Corn 
# Doce – Mars – Doce [2, 1] and [2, 2]; Doce – Mars – Moon – Doce [13, 1]; Doce – Mars – Sky – Doce [4, 1] and [4, 2]
model.addConstr(quicksum(a[i,2,1,"Corn",export] + a[i,13,1,"Corn",export] + a[i,4,1,"Corn",export] for i in S1 for export in PT)* 35000 
+ quicksum(a[i,2,2,"Corn",export] + a[i,4,2,"Corn",export] for i in S2 for export in PT) * 70000 >= 10000, "c15")

# Sky export Iron: 40000 
# Doce – Sky – Doce [1, 1] and [1, 2]; Doce – Moon - Sky - Doce [12, 1]; Doce – Mars – Sky – Doce  [4, 1] and [4, 2];
# Bom – Sky – Bom [5,1] and [5, 2]; Bom - Moon – Sky - Bom [17, 1]; Bom – Mars - Sky - Bom [8, 1] and [8, 2]
model.addConstr(quicksum(a[i,1,1,importType,"Iron"] + a[i,4,1,importType,"Iron"] + a[i,5,1,importType,"Iron"] + a[i,8,1,importType,"Iron"] + a[i,12,1,importType,"Iron"] + a[i,17,1,importType,"Iron"] for i in S1  for importType in PT)* 35000 + 
    quicksum(a[i,1,2,importType,"Iron"] + a[i,4,2,importType,"Iron"] + a[i,5,2,importType,"Iron"] + a[i,8,2,importType,"Iron"] for i in S2  for importType in PT) * 70000 >= 40000, "c16")


# Acabam em Sky – Wheat: 30 000
# Bom – Sky – Bom [5,1] and [5, 2]; Bom – Sky - Moon – Bom [15, 1]; Bom – Sky - Mars – Bom [7,1] and [7,2]
model.addConstr(quicksum(a[i,5,1,"Wheat",export] + a[i,15,1,"Wheat",export] + a[i,7,1,"Wheat",export] for i in S1 for export in PT)* 35000 + 
    quicksum(a[i,5,2,"Wheat",export] + a[i,7,2,"Wheat",export] for i in S2 for export in PT) * 70000 >= 30000, "c17")

# Export of Moon -  Copper: 10000
# Doce – Moon – Doce [9, 1]; Doce – Sky - Moon – Doce [10, 1]; Doce – Mars – Moon – Doce [13, 1]
model.addConstr(quicksum(a[i,9,1,"Copper", export] + a[i,10,1,"Copper", export] + a[i,13,1,"Copper", export] for i in S1 for export in PT)* 35000 >= 10000, "c18")

# Chegam a Moon – Corn: 30 000
# Doce – Moon – Doce [9, 1]; Doce – Moon - Mars - Doce [11, 1]; Doce – Moon - Sky - Doce [12, 1]
model.addConstr(quicksum(a[i,9,1,"Corn", export] + a[i,11,1,"Corn", export] + a[i,12,1,"Corn", export] for i in S1 for export in PT)* 35000 >= 10000, "c19")


# TODO: get the number of vessels for the variable a
# for vessel in range(1, M):
    # if a[vessel,..] = 0 ==> x[1] < vessel
    # model.addConstr(quicksum(a[vessel,i,1,importType, export] for i in S1 for export in PT for importType in PT)  >= x[1] - vessel + 1, "c20") # change
    # if a[vessel,..] > 0 ==> x[1] >= vessel
