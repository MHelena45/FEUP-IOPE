...
for tt in S1:
    for t in range(1, 3):
        # Número de viagens de cada tipo que se vai fazer e de que (ida e volta)
        for imp in PT:
            for exp in PT:
                t[tt, 1, imp, exp]=model.addVar(vtype="I", name="t(%s,%s,%s)" %(tt,1,imp,exp))
model.update()  # vars were added

for tt in range(1, S2):
    for t in range(1, 3):
        # Número de viagens de cada tipo que se vai fazer e de que (ida e volta)
        for imp in PT:
            for exp in PT:
                t[tt, 2, imp, exp]=model.addVar(vtype="I", name="t(%s,%s,%s)" %(tt,2,imp,exp))
model.update()  # vars were added

a = {}
# as viagens que cada navio vai fazer
for i in range(1, M):
    a[i, tt, 1] = model.addVar(vtype="I", name="t(%s,%s,%s)" %(tt,imp,exp,1))
    a[i, tt, 2] = model.addVar(vtype="I", name="t(%s,%s,%s)" %(tt,imp,exp,2))
model.update()  # vars were added
...
# duração da atribuição de viagens tem que ser menor que 365 - 20
# for each vehicle
for i in M:
    #makes sure that the trips assignee last less than the operation time
    model.addConstr(quicksum( t[tt,1,i,j] * a[i,tt,1] for tt in S1 for i in PT for j in PT) <= 345 * 24, "c5(%s,%s)" % (i,1))
    #the same for type 2
    model.addConstr(quicksum( t[tt,2,i,j] * a[i,j,2] for j in S2 for i in PT for j in PT) <= 345 * 24, "c6(%s,%s)" % (i,2))


## BOM
# viagens que saem de Bom relaciona-se com a carga de exportação - 5 a 8, 14 a 18
